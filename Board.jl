...
function drawBoard(ax:: Axis, board::Union{Board, Observable{Board}})
    ...
    ob = @lift( reshape(map(x -> lift(y -> y[x] == :red ? c4red : y[x] == :yellow ? c4yellow : y[x] != :empty ? y[x] : (:white, 0.0), board), eachindex($board)), 6, 7)) # Turns the observable board of chips into an observable board of observable colours which listens to the original board
